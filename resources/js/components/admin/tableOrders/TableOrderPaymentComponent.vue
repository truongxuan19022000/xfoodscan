            ...
            <div class="modal-body">
                <div class="mb-4">
                    <div
                        ...
                        <span class="text-primary text-base font-medium">{{ order.total_currency_price }}</span>
                    </div>
                </div>
                <div class="mb-4">
                    ...
                    <nav class="flex flex-wrap gap-4 active-group">
                        <button data-tab="#cash" type="button"
                            ...
                            :class="form.pos_payment_method === posPaymentMethodEnum.CASH ? 'active' : ''"
                        ...
                        <button data-tab="#card" type="button"
                            class="other-tabBtn w-fit flex flex-col items-center gap-2 rounded-lg py-3 px-7 border bg-[#F7F7FC] border-[#F7F7FC]"
                            :class="form.pos_payment_method === posPaymentMethodEnum.CARD ? 'active' : ''"
                        ...
                        <button data-tab="#mfs" type="button"
                            class="other-tabBtn w-fit flex flex-col items-center gap-2 rounded-lg py-3 px-7 border bg-[#F7F7FC] border-[#F7F7FC]"
                            :class="form.pos_payment_method === posPaymentMethodEnum.MOBILE_BANKING ? 'active' : ''"
                            @click="paymentMethod(posPaymentMethodEnum.MOBILE_BANKING); fetchQrCode()">
                        ...
                        <button data-tab="#otherpay" type="button" onclick="createkeyboard('otherpay')"
                            class="other-tabBtn w-fit flex flex-col items-center gap-2 rounded-lg py-3 px-7 border bg-[#F7F7FC] border-[#F7F7FC]"
                            :class="form.pos_payment_method === posPaymentMethodEnum.OTHER ? 'active' : ''"
                ...
                <div id="cash" class="data-tab hidden"
                    ...
                    <div class="mb-4">
                        ...
                        <input id="cashInput" ref="cashInput" type="text" v-bind:value="order?.cash_back_amount ? Math.abs(order?.cash_back_amount) : ''" v-on:keypress="floatNumber($event)"
                ...
                <div id="mfs" class="data-tab hidden"
                    ...
                    <div class="mb-4">
                        ...
                        <input id="mfs-trans" type="text" ref="mfsInput"
                            v-bind:value="order?.cash_back_amount ? Math.abs(order?.cash_back_amount) : ''"
                            @keyup.enter="fetchQrCode()"
                            class="h-12 w-full rounded-lg border py-1.5 px-4 placeholder:text-xs border-[#D9DBE9]">
                    </div>
                    <div class="mb-4 flex flex-col items-center">
                        <div v-if="qrLoading" class="flex items-center justify-center h-48">
                            <svg class="animate-spin h-10 w-10 text-primary" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"></path>
                            </svg>
                        </div>
                        <img v-else-if="qrCode" :src="qrCode" alt="QR Code" class="w-48 h-48 object-contain border rounded-lg" />
                        <p v-else-if="qrError" class="text-red-500 text-sm">{{ qrError }}</p>
                    </div>
                ...
                <div class="grid grid-cols-4 gap-x-4 gap-y-3.5 mb-6"
                    v-if="form.pos_payment_method === posPaymentMethodEnum.CASH || form.pos_payment_method === posPaymentMethodEnum.CARD">
                    ...
                    <button
                        :onclick="form.pos_payment_method === posPaymentMethodEnum.CASH ? `solve('.', '${inputIdName}')` : ''"
                        value="point"
                ...
                <button @click="confirmOrder" type="button"
                    class="rounded-3xl text-base py-2 px-3 font-medium w-full text-white bg-primary">{{
                        $t("label.confirm_payment") }}</button>
            </div>
        </div>
    </div>
</template>
...
export default {
    name: "TableOrderPaymentComponent",
    ...
    watch: {
        order: {
            immediate: true,
            handler(newOrder) {
                if (newOrder && newOrder.branch_id) {
                    this.$store.dispatch('branch/show', newOrder.branch_id).catch(() => {});
                }
            },
        },
    },
    methods: {
        ...
        paymentMethod: function (method, Idname = "") {
            ...
            this.form.pos_payment_method = method;
            this.form.pos_payment_note = "";
        },
        fetchQrCode: async function () {
            ...
            this.qrLoading = true;
            try {
                const rawInput = this.$refs.mfsInput ? this.$refs.mfsInput.value : '';
                const amountQRCode = parseInt(rawInput.replace(/[^0-9]/g, ''), 10) || Math.abs(this.order?.cash_back_amount) || 0;
                const branch = this.branch || {};
                const response = await fetch('https://api.vietqr.io/v2/generate', {
                    method: 'POST',
                    headers: {
                        'x-client-id': branch.vietqr_client_id || 'd41a9492-4deb-4d9f-be25-3da66533cc48',
                        'x-api-key': branch.vietqr_api_key || '9a442529-e13d-4b2c-96be-59618dc09c74',
                        'Content-Type': 'application/json',
                    },
                    body: JSON.stringify({
                        accountNo: branch.bank_account_no || '19022000888',
                        accountName: branch.shop_name || 'QUY VAC XIN PHONG CHONG COVID',
                        acqId: branch.bank_acq_id || '970423',
                        addInfo: branch.bank_transfer_info || 'Ung Ho Quy Vac Xin',
                        amount: String(amountQRCode),
                        template: 'compact2',
                    }),
                });
                const data = await response.json();
                if (data && data.data && data.data.qrDataURL) {
                    this.qrCode = data.data.qrDataURL;
                } else {
                    this.qrError = 'Không thể tạo mã QR. Vui lòng thử lại.';
                }
            } catch (err) {
                this.qrError = 'Lỗi kết nối tới VietQR. Vui lòng thử lại.';
            } finally {

<template>
    ...
    <div class="col-12">
        <div class="db-card p-4">
            <div class="flex flex-wrap gap-y-5 items-end justify-between">
                ...
                <div id="listBtn" class="flex flex-wrap gap-3" v-else-if="order.status !== enums.orderStatusEnum.REJECTED &&
                    order.status !== enums.orderStatusEnum.CANCELED
                ">
                    <button type="button" @click="orderSubmit" v-if="order.payment_status === enums.paymentStatusEnum.UNPAID"
                            class="flex items-center justify-center text-white gap-2 px-4 h-[38px] rounded shadow-db-card bg-primary">
                            <i class="lab lab-save"></i>
                            <span class="text-sm capitalize text-white">{{ $t("button.payment_confirm") }}</span>
                    </button>
    ...
    <TableOrderPaymentComponent :order="order" />
...
import TableOrderPaymentComponent from "./TableOrderPaymentComponent";

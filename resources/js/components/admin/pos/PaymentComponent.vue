            ...
            <div class="modal-body">
                ...
                <div id="cash" class="data-tab hidden"
                    ...
                    <div class="mb-4">
                        ...
                        <input id="cashInput" ref="cashInput" type="text" v-model="props.form.total" v-on:keypress="floatNumber($event)"
    ...
    mounted() {
        this.$store.dispatch('defaultAccess/show').then((res) => {
            if (res.data.data.branch_id) {
                this.$store.dispatch('branch/show', res.data.data.branch_id).catch(() => {});
            }
        }).catch(() => {});
    },
    methods: {
        ...
        fetchQrCode: async function () {
            ...
            try {
                ...
                const amountQRCode = parseInt(rawInput.replace(/[^0-9]/g, ''), 10) || Math.round(this.$props.props.form.total) || 0;
            ...
            try {
                ...
                this.$store.dispatch("defaultAccess/show").then((res) => {
                    ...
                    this.$store.dispatch('posOrder/save', this.$props.props.form).then(orderResponse => {
                        this.$props.props.form.token = new Date().toISOString().slice(0,10).replace(/-/g, '') + Math.floor(Math.random() * 9990 + 10).toString();
                        ...
                        if (err.response && err.response.data && typeof err.response.data.errors === 'object') {
                            ...
                            alertService.error(err.message || err);
                        }
                    });
                }).catch((err) => {
                    this.loading.isActive = false;

...
class OfferRequest extends FormRequest
{
    ...
    {
        return [
            'amount.required' => trans('all.message.discount_percentage_required')
        ];
    }
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            if (!$this->isNotNull(request('start_date'))) {
                $validator->errors()->add('start_date', trans('all.message.start_date_required'));
            }

            if (!$this->isNotNull(request('end_date'))) {
                $validator->errors()->add('end_date', trans('all.message.end_date_required'));
            }

            if ($this->isNotNull(request('start_date')) && strtotime(request('end_date')) < strtotime(request('start_date'))) {
                $validator->errors()->add('end_date', trans('all.message.end_date_invalid'));
            }
            if ($this->isNotNull(request('start_date')) && $this->checkToDate()) {
                $validator->errors()->add('end_date', trans('all.message.end_date_past'));
            }
        });
    }

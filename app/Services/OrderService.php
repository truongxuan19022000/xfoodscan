...
class OrderService
{
    ...
    public function list(PaginateRequest $request)
    {
        try {
            ...
            return Order::with('transaction', 'orderItems', 'branch', 'user', 'diningTable')->where(function ($query) use ($requests) {
            ...
            DB::transaction(function () use ($request) {
                $this->order = Order::create(
                    $request->validated() + [
                        ...
                        'token'            => $request->token,
                        'payment_status'   => $request->pos_payment_method != PosPaymentMethod::OTHER ? PaymentStatus::PAID : PaymentStatus::UNPAID,
                        'order_datetime'   => date('Y-m-d H:i:s'),
    ...
    public function salesReportOverview(Request $request)
    {
        try {
            ...
            $total_sale = AppLibrary::flatAmountFormat(
                Order::where(function ($query) use ($requests) {
                    if (isset($requests['from_date']) && isset($requests['to_date'])) {
                        $first_date = Date('Y-m-d', strtotime($requests['from_date']));
                        $last_date  = Date('Y-m-d', strtotime($requests['to_date']));
                    } else {
                ...
                })->where('payment_status', PaymentStatus::PAID)->sum('total')
            );
            $salesReportArray['total_sales'] = AppLibrary::currencyAmountFormat($total_sale);

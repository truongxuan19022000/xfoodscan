...
class ItemService
{
    ...
    {
        try {
            return Cache::remember('item_show_' . $item->id, now()->addMinutes(10), function () use ($item) {
    ...
    {
        try {
            return Cache::remember('featured_items', now()->addMinutes(5), function () {
    ...
    {
        try {
            return Cache::remember('most_popular_items', now()->addMinutes(10), function () {
    ...
    {
        return Cache::remember('item_details_' . $item->id, now()->addMinutes(10), function () use ($item) {
            return $item->load('media', 'category', 'tax', 'offer', 'addons', 'variations', 'extras');
        });
    }
     ...
     * When $itemId is provided, clears specific item caches.
     * Always clears collection caches (featured, popular).
     */
    public static function clearItemCaches(?int $itemId = null): void
    {
        // Clear collection caches
        Cache::forget('featured_items');
        Cache::forget('most_popular_items');

        // Clear specific item caches
        if ($itemId) {
            Cache::forget('item_details_' . $itemId);
            Cache::forget('item_show_' . $itemId);
        }
    }
}
